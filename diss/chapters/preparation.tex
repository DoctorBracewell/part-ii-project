...
\label{sec:2.1}
Pursuit-evasion problems - where agents compete to capture or escape one another - were first formalised through a framework for reasoning about intelligent adversaries by Isaacs\cite{isaacs1965} and remain an active area of research across robotics, autonomous systems and aerospace\cite{weintraub2020}.

Determining optimal strategies in real-time or close-to real time is computationally challenging as the problem grows in dimensionality - the state-action space becomes too large to represent with traditional exact methods\cite{bertsekas1996,powell2007}. This ‘curse of dimensionality’\cite{bellman1957} motivates a variety of approximation approaches. McGrew et al.\cite{mcgrew2010} demonstrated an approximate dynamic programming (ADP) approach to 1v1 air combat, involving a number of additional reward-shaping techniques. While effective, this approach is limited to 2D level flight at fixed velocity, does not approach the infinite state space of continuous flight\cite{bertram3d}, and does not scale to multi-agent teams. These limitations motivate a different approach for multi-agent pursuit-evasion in a continuous 3D environment.
