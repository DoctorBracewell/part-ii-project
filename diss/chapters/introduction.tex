...
\begin{enumerate}[noitemsep]
    ...
    \item Implement the FastMDP algorithm from scratch to drive behaviours between competing or collaborating agents.
    \item Design and investigate reward functions that produce valid pursuit-evasion behaviour.
...
\begin{enumerate}[resume,noitemsep]
    \item Scale the simulation from 1v1 encounters to multi-agent teams of arbitrary size.
    \item Demonstrate that the simulation and decision-making framework is applicable to alternative problem spaces beyond pursuit-evasion, requiring changes only to the reward structure.
\end{enumerate}
...
\begin{enumerate}
    \item \textbf{Behavioural validity}. The implemented FastMDP algorithm produces valid pursuit-evasion behaviour, defined according to a series of per-timestep constraints. This is the primary functional goal of the project and directly validates the correctness of the reward functions and decision-making framework.
    \item \textbf{Kinematic validity}. Agent kinematics remain within physically realistic bounds throughout all simulation runs, including velocity, turn rate and altitude constraints. This validates the correctness of the kinematic model independently of the decision-making layer.
